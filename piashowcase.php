...
function conditionally_add_piashowcase_scripts_and_styles($posts){
	...
	if ($shortcode_found) {
		...
		wp_register_script( "piashowcase", path_join(WP_PLUGIN_URL, basename( dirname( __FILE__ ) )."/js/piashowcase.js"), array('jquery') );
		wp_enqueue_script( "piashowcase");
...
class espiaaff_widget_info extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		$feed = 'feed://equalserving.com/client_area/durovy/affiliate_manager/';
		$feed = 'feed://postinstitute.com/affiliate/product/feed/';
		if (isset($prd_cd) && trim($prd_cd) != "") {
			$feed .= "?prd_cds=".$prd_cd;
		}
		//echo "<li>prd_cds = [".$prd_cd."]  Feed = [".$feed."]</li>";
		$rss = fetch_feed($feed);
		//print_r($rss);
		if (!is_wp_error( $rss ) ) : // Checks that the object is created correctly
